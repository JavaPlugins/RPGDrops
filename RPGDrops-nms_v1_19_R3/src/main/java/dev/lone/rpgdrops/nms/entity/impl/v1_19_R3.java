...
@Implementation.CyclicDependency(type = IEntityNms.class, version = Version.v1_19_R3)
public class v1_19_R3 implements IEntityNms<ClientboundSetPassengersPacket, net.minecraft.world.entity.Entity>
{
    @Override
    ...
    public void destroy(Entity bukkitEntity)
    {
        ...
        ((ServerChunkCache) handle.getLevel().getChunkSource()).chunkMap.entityMap.get(handle.getId()).seenBy.forEach(connection -> {
            ClientboundRemoveEntitiesPacket packet = new ClientboundRemoveEntitiesPacket(bukkitEntity.getEntityId());
            Packets.ignored.add(packet);
            connection.send(packet);
        });
    ...
    {
        ((ServerChunkCache) handle.getLevel().getChunkSource()).chunkMap.broadcast(handle, packet);
    }
}
